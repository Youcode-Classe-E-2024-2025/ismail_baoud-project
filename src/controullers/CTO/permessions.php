...
if ( isset( $_POST[ 'btn_role' ] ) ) {
    $role = htmlspecialchars( trim( $_POST[ 'role_create' ] ) );

    $existingRoles = ( new role_permession() )->role( $role );
    if ( empty( $existingRoles ) ) {
        $res = new role_permession();
        $res->add_roles( $role );
        $_SESSION[ 'role_success' ] = 'Role created successfully.';
    } else {
        $_SESSION[ 'role_error' ] = 'Role already exists.';
